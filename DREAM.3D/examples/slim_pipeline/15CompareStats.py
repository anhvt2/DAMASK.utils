...
output_path = "pipeline_output/15-compare_statistics"

data = {
    "files":{
        "Prior Particles":{
            # "EBSD YZ Small Plane":"pipeline_output/7-xy-statistics.dream3d",
            "EBSD YZ Small Plane":"pipeline_output/7-xy-statistics-StatsGenerator.dream3d",
            "Synthetic":"pipeline_output/14-synthetic_statistics.dream3d"
        }
    },
    "statistics":{
        "MisorientationList":{
...
#            axis.hist(x=data_axis,bins=bins,weights=weights_axis,label=label,alpha=alpha)
            axis.hist(x=data_axis,bins=HIST_BINS,weights=weights_axis,label=label,alpha=alpha)
            axis.legend()
        else:
...
def compare_statistics(inputs,alpha,save):
    ...
    for domain in domains:
        ...
        for statistic in statistics:
            ...
            if save:
                plt.savefig(output_path+"_"+statistic+".png")
            else:
                plt.show()
