...
class SandwichBeam:
    ...
    def __init__(self, Lx=1.0, Ly=0.25, hex=0.025, hey=0.025, E1=200e9, E2=100e9, nu=0.25):
        self.Lx = Lx # beam width
        self.Ly = Ly # beam height
        self.hex = hex # element size in x-direction
        self.hey = hey # element size in y-direction

        self.nelx = int(Lx/hex) # number of elements in the x-direction
        self.nely = int(Ly/hey) # number of elements in the y-direction
        if not self.nely % 5 == 0:
            raise ValueError("nely = Ly/hy must be dividable by 5 for the current \
    ...
    def compute_displacement(self):
        ...
        q = self.hex/self.hey
        A11 = np.array([[8/q + 4*q,  3, -8/q + 2*q, -3], [ 3, 4/q + 8*q,  3,  -4/q + 4*q], \
                [-8/q + 2*q,  3, 8/q+4*q, -3], [-3,  -4/q+4*q, -3, 4/q+8*q]])
        A12 = np.array([[-4/q-2*q, -3,  4/q-4*q,  3], [-3, -2/q-4*q, -3, 2/q-8*q], \
                [ 4/q-4*q, -3, -4/q-2*q,  3], [ 3, 2/q-8*q,  3, -2/q-4*q]])
        B11 = np.array([[-4*q,  3, -2*q,  9], [ 3, -4/q, -9,  4/q], [-2*q, -9, -4*q, -3], \
                [ 9,  4/q, -3, -4/q]])
        B12 = np.array([[ 2*q, -3,  4*q, -9], [-3,  2/q,  9, -2/q], [ 4*q,  9,  2*q,  3], \
                [-9, -2/q,  3,  2/q]])
    ...
    def compute_stress(self):
        ...
        # B-matrices
        dN1x12 = -1/self.hex; dN2x12 = 1/self.hex; dN3x12 = 0; dN4x12 = 0;
        dN1x34 = 0; dN2x34 = 0; dN3x34 = 1/self.hex; dN4x34 = -1/self.hex;
        dN1y14 = -1/self.hey; dN2y14 = 0; dN3y14 = 0; dN4y14 = 1/self.hey;
        dN1y23 = 0; dN2y23 = -1/self.hey; dN3y23 = 1/self.hey; dN4y23 = 0;
    ...
    def plot_displacement(self):
        ...
        scale = max(self.hex*self.nelx,self.hey*self.nely)/(10*max(abs(self.u)))
...
def main():
    ...
    parser.add_argument("-x", "--elemx", dest="hex", type=float, default=0.025, \
            help="element size in x-direction")
    parser.add_argument("-y", "--elemy", dest="hey", type=float, default=0.025, \
            help="element size in y-direction")
    parser.add_argument("-E", "--young", dest="E", type=float, nargs=2, \
    ...
    sw = SandwichBeam(hex=args.hex, hey=args.hey, E1=args.E[0], E2=args.E[1])
