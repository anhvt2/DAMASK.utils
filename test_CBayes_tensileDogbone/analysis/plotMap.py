    ...
    for poroType, poroIdx in zip(['global','local'], [0,1]):
        ...
        for mltype, color, alpha, marker in zip(['train/', 'test/test-run-437-', 'test/test-run-180-', 'test/test-run-90-', 'test/test-run-20-'], ['tab:gray', 'tab:red', 'tab:green', 'tab:orange', 'tab:blue'], [0.75, 0.75, 0.65, 0.55, 0.45], ['o','s','p','h','D']):
            for folderName in glob.glob(f'{mltype}*/'):
                ...
                # idxStressStrain = 2 # see interp_stress_strain.log
        ...
        plt.title(r'$\phi \mapsto \sigma | \varepsilon$ at fixed $\varepsilon = %.4f$' % interpStrain, fontsize=24)
        plt.savefig(f'ConditionalStress-Porosity-{poroType}-{idxStressStrain}.png', dpi=300, facecolor='w', edgecolor='w', orientation='portrait', format=None, transparent=False, bbox_inches='tight', pad_inches=0.1, metadata=None)
        plt.clf()
