...
geomFileName="padded_voidSeeded_1.500pc_spk_dump_12_out.geom" # ${geomFileName}.geom
loadFileName="tension" # ${loadFileName}.load

# how to run in quiet mode: rm -f nohup.out; nohup 
postResults --cr fp,f,p,grainrotation,texture --split --separation x,y,z --increments --range 1 44 1 ${geomFileName}_${loadFileName}.spectralOut 2>&1 > log.postResults
