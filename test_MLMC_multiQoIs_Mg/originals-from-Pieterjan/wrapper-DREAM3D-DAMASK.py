...
def main():
    ...
    if not args.level is None:
        ...
        print(f"Collocated von Mises stresses at {level} is {sample()}")
        if level > 0:
            print(f"Collocated von Mises stresses at {level - 1} is {sample()}")

    # 2d multi-index
    if not args.index is None: # works for 2d only
        index = list(map(int, args.index[1:-1].split(", ")))
        print(f"Collocated von Mises stresses at ({index[0]}, {index[1]}) is {sample()}")
        if index[0] > 0:
            print(f"Collocated von Mises stresses at ({index[0] - 1}, {index[1]}) is {sample()}")
        if index[1] > 0:
            print(f"Collocated von Mises stresses at ({index[0]}, {index[1] - 1}) is {sample()}")
        if index[0] > 0 and index[1] > 0:
            print(f"Collocated von Mises stresses at ({index[0] - 1}, {index[1] - 1}) is {sample()}")
