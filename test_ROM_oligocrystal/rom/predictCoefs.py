...
for foi, startId in zip(fois, startIds):
    ...
    np.save('outputRom_Pred_%s', y_test_pred)
...
header = ['podCoef-MisesCauchy-%d' % i for i in range(1,numFtrs+1)] + ['podCoef-MisesLnV-%d' % i for i in range(1,numFtrs+1)] # output file header

np.savetxt('outputRom_Pred', delimiter=',', header=header, comments='')
