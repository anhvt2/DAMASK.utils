...
NumFtr  = 300 # number of ROM/POD features
...
for foi, startId, model in zip(fois, startIds, [NNRegressor_MisesCauchy(), NNRegressor_MisesLnV()]):
    ...
    y_train = np.loadtxt('outputRom_Train.dat', delimiter=',', skiprows=1)[:,startId:startId+NumFtr]
    y_test  = np.loadtxt('outputRom_Test.dat',  delimiter=',', skiprows=1)[:,startId:startId+NumFtr]
...
headerStr = ['podCoef-MisesCauchy-%d' % i for i in range(1,NumFtr+1)] + ['podCoef-MisesLnV-%d' % i for i in range(1,NumFtr+1)] # output file header
...
for i in range(NumCases):
    ...
    tmpCoefs[:NumFtr,0] = predCoefs_MisesCauchy[i,:]
    tmpCoefs[:NumFtr,1] = predCoefs_MisesLnV[i,:]
