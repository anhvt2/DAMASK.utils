...
"""
    check_variances_checkpoint([max_level=3], [budget=10])

Take warm-up samples at levels 0 to `max_level` to check for
sufficient variance decay. Takes `budget` samples on each index.
"""
function check_variances_checkpoint(; max_level=3, budget=10)

    # create estimator
    estimator = Estimator(ML(), # Multilevel
						  MC(), # Monte Carlo
						  () -> (), # No sample function (use checkpoints)
						  get_distributions(), # Distributions for the unknown parameters
						  checkpoint=true, # Use checkpoints (i.e., write samples to file)
						  samples_dir="samples_for_variance_decay" # Directory where to save samples
						 )
    ...
    for level in 0:max_level
		ell = Level(level)
		push!(estimator, ell)
        flag += MultilevelEstimators.sample!(estimator, ell, budget)
    end

    if flag == 0 # All samples are available
        for level in 0:max_level
			ell = Level(level)
            V = MultilevelEstimators.var0(estimator, ell)
            dV = MultilevelEstimators.var(estimator, ell)
            N = MultilevelEstimators.nb_of_samples(estimator, ell)
            println("Level ", ell, ", V = ", V, ", dV = ",
                dV, " (", N, " samples)")
        end
    end
end
...
# Call this function to check for variance decay using the checkpointed code.
check_variances_checkpoint()
