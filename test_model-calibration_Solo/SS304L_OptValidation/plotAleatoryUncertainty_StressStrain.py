...
sveIndexList = range(1,6)
markerList = ['^', 'v', 'd', 's', 'o']
colorList = ['k', 'm', 'purple', 'r', 'b']
array_interp_vareps = np.zeros([100, len(sveIndexList)]) # 100 depends on discretization of getInterpStressStrain()
array_interp_sigma  = np.zeros([100, len(sveIndexList)]) # 100 depends on discretization of getInterpStressStrain()

for i in range(len(sveIndexList)): # mesh-resolution index
	StressStrainFile = 'sve%d_8x8x8/postProc/stress_strain.log' % (i+1)
	...
	ax.plot(interp_vareps, interp_sigma, linestyle=':', c=colorList[i], marker=markerList[i], label='        ' ) # label='SVE %d' % (i+1))
...
plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0, markerscale=3, fontsize=72, frameon=False)

plt.xlabel(r'$\varepsilon$ [-]', fontsize=30)
plt.ylabel(r'$\sigma$ [MPa]', fontsize=30)
plt.title(r'Aleatory uncertainty from RVE on homogenized $\sigma-\varepsilon$ curves (spectral CPFEM)', fontsize=24)
