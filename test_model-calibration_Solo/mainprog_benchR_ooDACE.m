...
% %% define objective GPR params -- default -- settings for DACE toolbox
% % initial guess for theta hyper-parameters
% theta = 1 * ones(1,d);
% lob = 2.5e-1 * ones(1,d);
% upb = 7.5e+0 * ones(1,d);

% %% feasibility classif-GPR -- default -- settings for DACe toolbox
...
cd ('./ooDACE'); startup; cd(parentPath); % add ooDACE toolbox; % add ooDACE toolbox % addpath(strcat(parentPath,'/dace')); % add DACE toolbox;
...
%% setting for ooDACE toolbox
blindGpOoDaceOpts = struct();
blindGpOoDaceOpts.type = 'BlindKriging';
blindGpOoDaceOpts.retuneParameters = true;
blindGpOoDaceOpts.xLB = xLB; 
blindGpOoDaceOpts.xUB = xUB;

gpOoDaceOpts = rmfield(blindGpOoDaceOpts, 'type');
gpOoDaceOpts.corrFunc = @corrmatern32;
...
tic;
fprintf('\n\nFitting dmodel...\n\n\n');
% [dmodelInterp, ~] = dacefit(S(F>0,:), Y(F>0), @regpoly0, @corrgauss, theta, lob, upb, xLB, xUB); % DACE
% try
%	dmodelInterp = oodacefit( S(F>0,:), Y(F>0), blindGpOoDaceOpts ); % ooDACE
% catch
	dmodelInterp = oodacefit( S(F>0,:), Y(F>0), gpOoDaceOpts ); % ooDACE
end
for i = 1:length(Y)
	% if F(i) == 0, Y(i) = predictor(S(i,:), dmodelInterp); end % DACE
	if F(i) == 0, Y(i) = dmodelInterp.predict(S(i,:)); end % ooDACE
end
clear dmodelInterp;
% [dmodel, ~] = dacefit(S, Y, @regpoly0, @corrgauss, theta, lob, upb, xLB, xUB); % DACE
try
	dmodel = oodacefit( S, Y, blindGpOoDaceOpts ); % ooDACE
catch
	dmodel = oodacefit( S, Y, gpOoDaceOpts ); % ooDACE
end

% [dmodelClassif, ~] = dacefit(S, F, @regpoly0, @correxp, thetaClassif, lobClassif, upbClassif, xLB, xUB); % DACE
dmodelClassif = oodacefit( S, F, gpOoDaceOpts ); % ooDACE

%% after interpolation -- write to files; will be used in bayesOptSrc/getBatch.py
% rescale
...
for i = (numParallelPoint + 1):maxiter
	...
	if batchID == 1
		...
		x = getNextSamplingPoint(dmodel, dmodelClassif, acquisitionFunction, blindGpOoDaceOpts); % batch acquisition
	elseif batchID == 2
		x = getNextSamplingPointReduceMSE(dmodel, blindGpOoDaceOpts); % batch explore
	elseif batchID == 3
		x = getNextSamplingPointReduceMSE(dmodelClassif, blindGpOoDaceOpts); % batch exploreClassif
	elseif batchID == 4
