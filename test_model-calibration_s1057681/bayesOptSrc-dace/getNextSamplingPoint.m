function xNext = getNextSamplingPoint(dmodel, dmodelClassif, acquisitionFunction)
	...
	multipleDesignSitesViolator = checkMultipleDesignSites(xNext, dmodel.origS); % return 1 if duplicated, 0 if not
	...
	while ~constraintViolator || multipleDesignSitesViolator
		...
		multipleDesignSitesViolator = checkMultipleDesignSites(xNext, dmodel.origS);
		fprintf('getNextSamplingPoint.m: constraintViolator = %d\n', constraintViolator);
		fprintf('getNextSamplingPoint.m: multipleDesignSitesViolator = %d\n', multipleDesignSitesViolator);
	end
