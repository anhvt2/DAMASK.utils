function xNext = getNextSamplingPoint(hyp, meanfunc, covfunc, likfunc, hypClf, meanfuncClf, covfuncClf, likfuncClf, acquisitionFunction, xLB, xUB, S, Y, F)
	...
	OPTS.Restarts = 3;
	% run mode
	OPTS.MaxIter = 250; % set MaxIter
	OPTS.MaxFunEvals = 250; % set MaxFunEvals
	...
	% xNext = cmaes('calculateNegativeAcquisitionFunction', xInitCMAES, sigma, OPTS, hyp); 
	% xNext = cmaes('calculateNegativeAcquisitionFunction', xBest, sigma, OPTS, hyp); 
	% xNext = cmaes('calculateNegativeAcquisitionFunction', 0.5 * (xBest + xInitCMAES), sigma, OPTS, hyp); 
	% xNext = cmaes('calculateNegativeAcquisitionFunction', w * xBest + (1 - w) * xInitCMAES, sigma, OPTS, hyp); % original
	...
	w = rand(); 
	xNext = cmaes('calculateNegativeAcquisitionFunction', w * xBest + (1 - w) * xInitCMAES, sigma, OPTS, hyp, meanfunc, covfunc, likfunc, hypClf, meanfuncClf, covfuncClf, likfuncClf, acquisitionFunction, xLB, xUB, S, Y, F);
	constraintViolator = checkConstraint(xNext); % return 0 if violated; 1 if not
	multipleDesignSitesViolator = checkMultipleDesignSites(xNext, S); % return 1 if duplicated, 0 if not
	...
	while ~constraintViolator || multipleDesignSitesViolator
		...
		multipleDesignSitesViolator = checkMultipleDesignSites(xNext, S);
		fprintf('getNextSamplingPoint.m: constraintViolator = %d\n', constraintViolator);
		fprintf('getNextSamplingPoint.m: multipleDesignSitesViolator = %d\n', multipleDesignSitesViolator);
	end
