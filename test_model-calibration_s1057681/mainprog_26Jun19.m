...
for i = (numParallelPoint + 1):maxiter
	...
	cd(parentPath); 
	system('python bayesSrc/checkComplete.py');
	system('python bayesSrc/getBatch.py'); % return the batchID for next query; dump to batchID.dat
	batchID = dlmread('batchID.dat');
	...
	system('bash ../querySpk.sh'); % end-to-end queryX.sh: from input.dat to output.dat, feasible.dat, complete.dat, rewards.dat
