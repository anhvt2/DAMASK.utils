...
# parser.add_argument("-maxWorkers", "--maxWorkers", type=int, default=None)
parser.add_argument("-isAnnotated", "--isAnnotated", type=bool, default=False)
args = parser.parse_args()
# maxWorkers = args.maxWorkers
isAnnotated = bool(args.isAnnotated)
...
maxWorkers = numWorkers # number of effective workers; to be updated later in the script
...
for i in range(n):
	...
	# safeguard if stopTimeObjs[i] < startTimeObjs[i]
	if (stopTimeObjs[i] - startTimeObjs[i]).total_seconds() < 0:
		print('stopTimeObjs[%d] < startTimeObjs[%d]: check folder %s' % (i, i, folderName[i]))
...
for j in range(numWorkers):
	...
	for i_work in range(num_of_works):
		...
		elif acquisition[i_iteration] == '-1': # MSE
...
if maxWorkers == numWorkers:
	ax.set_ylim([numWorkers + 0.5, -0.5])
else:
	ax.set_ylim([maxWorkers - 0.5, -0.5])
...
print('\n\n')
print('---------------------------------------')
print('refStartTime =', refStartTime)
print('refStopTime =', refStopTime)
print('numInitPoint =', numInitPoint)
print('---------------------------------------')
