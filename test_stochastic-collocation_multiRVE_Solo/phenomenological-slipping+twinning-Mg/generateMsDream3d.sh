...
elif [[ ${hostName} == *"asus"* ]]; then # Asus Server
	execPath="/media/anhvt89/hsgt2Repo/DREAM.3D/DREAM3D-6.5.141-Linux-x86_64/bin"
else
...
defaultPath=$(grep -inr 'OutputPath' ${inputFile}  | head -n 1  | cut -d: -f3 | cut -c 3- | rev | cut -c 3- | rev)
