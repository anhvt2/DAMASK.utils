...
dakotaInputFile='textbook7d_uq_sc_pyImport.in' # input file
if [[ "${qoi}" == "strain" ]]; then 
	qoiIndex=0; 
fi
if [[ "${qoi}" == "stress" ]]; then
...
sed -i "44s|.*|o_ = outputData[index_, ${qoiIndex}] # change the second index accordingly: 0 = strainYield, 1 = stressYield|" damask_query.py
