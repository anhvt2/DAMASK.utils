...
def renumerate(geom, startIndex=0, cluster=False):
    ...
    if cluster == True:
        ...
        for i in range(len(grainIdxList)):
            ...
            # re-start at 0: no noisy samples
            clustering.labels_ -= np.min(clustering.labels_)
            for j in range(clustering.labels_.shape[0]):
                renumeratedGeom[x[j], y[j], z[j]] = maxGrainId + \
                    clustering.labels_[j]+startIndex+1
            # Print diagnostics
            logging.info(f'clustering.labels_ = {clustering.labels_}')
            logging.info(
                f'np.min(clustering.labels_) = {np.min(clustering.labels_)}')
            logging.info(
                f'np.max(clustering.labels_) = {np.max(clustering.labels_)}')
            logging.info(f'maxGrainId = {maxGrainId}')
            logging.info(
                f'renumerate(): Segregating grains from grainId {grainIdx} to [{maxGrainId+np.min(clustering.labels_)+startIndex+1}, {maxGrainId+np.max(clustering.labels_)+startIndex+1}].')
            logging.info(f'\n')
            # Update maxGrainId
        ...
        for i in range(len(grainIdxList)):
            ...
            x, y, z = np.where(geom == grainIdx)
            logging.info(
                f'renumerate(): Mapping grain id from {grainIdx} to {startIndex+i}.')
...
# This doesn't renumerate but only clustering
